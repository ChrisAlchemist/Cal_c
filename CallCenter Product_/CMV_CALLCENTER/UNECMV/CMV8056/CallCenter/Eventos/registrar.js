            ...
            $.ajax({
                ...
                success: function (socio) {
                    ...
                    if (socio[0].estatus == 1)
                    {
                        if (usuarioCallCenter === true) {
                            $.ajax({
                                type: "POST", url: rootUrl("/Registrar/ValidaSocioBanca"), data: { numero: numero }, dataType: "json",
...
function cargarSupuestos(id) {
    ...
    if (id === "4") {
        ...
        $.ajax({
            type: "POST", url: rootUrl("/Registrar/ValidaSocioBanca"), data: { numero: $("#inputSocio").val() }, dataType: "json",
            success: function (valida) {
                if (valida === true) {
...
function validaContinuar() {
    ...
    if (esSocio === "0") {
        ...
        if ($("#inputNumeroCuentaPtmo").val() === "" && $("#selectReporte").val() !== "4") {
            validaForm = false;
...
function ValidaRequiereAutenticacion() {
    $.ajax({
        type: "POST", url: rootUrl("/Registrar/validaAutenticacionBanca"), data: { idSupuesto: idSupuesto }, dataType: "json",
        success: function (validaAutenticacionBanca) {
            if (validaAutenticacionBanca) {
                $("#dvLoading").removeAttr("hidden");
                $.ajax({
                    type: "POST", url: rootUrl("/Registrar/CargarCuestionario"), data: { numero: $("#inputSocio").val() }, dataType: "json",
                    success: function (validaAutenticacionBanca) {
            ...
            $.ajax({
                ...
                success: function (valida) {
                    if (valida === true) {
                        ...
                        $.ajax({
                            type: "POST", url: rootUrl("/Registrar/obtenerCuentas"), data: { tipoCuenta: $("#selectPtmoCapt").val(), numero: $("#inputSocio").val() }, dataType: "json",
...
function prueba() {
    $.ajax({
        type: "POST", url: rootUrl("/Registrar/Prueba"), data: {}, dataType: "json",
        success: function (diasDif) {
